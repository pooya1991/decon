...
for p in xrange(P):
    ...
    if lens[p] < clen:
        diff = clen - lens[p]
        padlen = int(math.ceil(float(diff) / 2))
        pad = np.zeros(padlen)
    ...
    if newmint >= 0 and smoothed[0] >= smoothed[1] and smoothed[0] != 0:
        # maxima[p].append( (newmint,smoothed[0]) )
...
if True:
    ...
    apeakfile = bfile.replace('.txt',
                              '_intensities_annfile.txt')  # Like afile, but contains the info about where the elution peak is
